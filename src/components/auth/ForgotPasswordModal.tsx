...
import { Button, GlassCard, Input, Spinner } from '../ui';
...
export interface ForgotPasswordModalProps {
  isOpen: boolean;
  onClose: () => void;
  /** After temp password is emailed */
  onSuccess: () => void;
  /** Shown when user landed from an old email link (`/forgot-password?token=`) */
  showLegacyLinkNotice?: boolean;
}
...
export function ForgotPasswordModal({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center p-0 sm:items-center sm:p-4">
      <div className="absolute inset-0 bg-black/50 backdrop-blur-md touch-manipulation" onClick={onClose} aria-hidden />
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="forgot-modal-title"
        className="relative m-0 max-h-[min(88dvh,100vh)] w-full max-w-md overflow-y-auto overscroll-y-contain rounded-t-2xl border border-gray-200/90 bg-white shadow-2xl sm:m-4 sm:rounded-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="sticky top-0 z-10 flex items-start justify-between gap-3 border-b border-gray-100 bg-white/95 px-5 py-4 backdrop-blur-sm sm:px-6 sm:py-5">
          <div className="min-w-0">
            <h2 id="forgot-modal-title" className="text-lg font-semibold text-gray-900">
              Forgot password
            </h2>
            <p className="mt-1 text-sm leading-snug text-gray-600">{subtitle}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="space-y-5 p-5 sm:p-6">
          {showLegacyLinkNotice && (
            <GlassCard variant="plain" className="!bg-amber-50/60 !border-amber-200 p-3">
              <p className="text-center text-sm text-amber-900">
                Password reset uses a verification code sent to your email. Continue below with your account email.
              </p>
            </GlassCard>
          )}
            ...
            <div className="space-y-5">
              <GlassCard variant="plain" className="p-4">
                <p className="text-center text-sm leading-relaxed text-gray-700">
                  ...
                  <strong>{OTP_EXPIRES_MINUTES} minutes</strong>. Nothing changes on your account until you enter the
                  code correctly.
                </p>
              </GlassCard>
            ...
            <div className="space-y-5">
              ...
              {expiresAtMs != null && (
                <GlassCard variant="plain" className="flex items-center justify-center gap-2 p-3">
                  ...
                  <p className={clsx('text-sm font-medium', codeExpired ? 'text-red-700' : 'text-gray-800')}>
